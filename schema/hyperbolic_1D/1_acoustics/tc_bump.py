...
class D1_acoustics_Bump(HashBaseModel):
    ...
    def ref_solution(self, t, x, field=None):
        xloc_l = self.xmin + (x + self.c*t - self.xmin) % (self.xmax-self.xmin)
        xloc_r = self.xmin + (x - self.c*t - self.xmin) % (self.xmax-self.xmin)
        sol_rho = .5 * (
            (1 + self.u_zero/self.c) * bump_init(
                xloc_r,
                self.reg,
                self.x_bump, self.width_bump,
                self.rho_ground, self.rho_bump
